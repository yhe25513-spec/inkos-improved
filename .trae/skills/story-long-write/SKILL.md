...
> **⚠️ 双系统兼容说明**：上述结构为本 skill 默认格式。如果项目是由 webnovel-writer 插件初始化的，则使用不同结构：`chapters/`（替代 `正文/`，文件名格式 `000X_章名.md`）、`story/`（替代 `大纲/`+`设定/`+`追踪/`，含 `story/outline/`、`story/roles/`、`story/runtime/` 等子目录）。**在任何文件操作前，必须先用 `ls` 检测实际存在的目录，不能假设哪种结构。** 如果检测到 `chapters/` 目录存在，所有写作输出写入 `chapters/`；如果检测到 `正文/` 目录存在，写入 `正文/`。两者都不存在时，按本文件默认格式创建。
...
12. **更新追踪**：写完后即时更新跟踪文件（根据项目结构检测选择实际路径）：
   - 伏笔文件（优先 `story/pending_hooks.md`，回退 `追踪/伏笔.md`）（新增/回收伏笔）
   - 时间线/状态文件（优先 `story/current_state.md`，回退 `追踪/时间线.md`）（记录事件时序）
   - 角色状态文件（优先 `story/current_state.md` + 对应角色文件，回退 `追踪/角色状态.md`）（如本章引起角色状态变化——身份、能力、关系、公众形象——则更新对应角色条目并追加变更记录）
   - **章节索引**：如果项目使用 `chapters/index.json`，每章完成后更新对应章节条目（`status`、`wordCount`、`updatedAt`、`lengthTelemetry.finalCount`）
   本章若首次引入会复用的具名角色/势力，按 Phase 3「细纲后设定补全」规则补建对应角色/势力档案（优先 `story/roles/{类型}/{角色名}.md`，回退 `设定/角色/{角色名}.md`）。角色状态更新规则详见 state-tracking.md。
...
**确定性收尾**：本批正文写完后，对所有新写正文文件先运行 `node .agents/skills/story-long-write/scripts/normalize-punctuation.js $CHAPTER_FILE`（写模式，默认 `--quote-mode keep`），再运行 `node .agents/skills/story-long-write/scripts/check-ai-patterns.js --check $CHAPTER_FILE`（`$CHAPTER_FILE` 为实际章节文件路径，如 `chapters/0002_龟兔与废铁.md`）。前者确定性清除正文里的 `……`、破折号 `——`/`—`、双连字符 `--` 和独立行 `---`，防止长篇累积横线和长省略；后者只报告先否定再肯定的高危 AI 句式，命中时必须回正文改掉并复扫到 0。对话打断、拖长音与数字区间不设破折号例外；盐言「」引号不受影响。narrative-writer agent 不运行这些脚本，由主会话在 agent 返回后针对实际落盘文件运行。
