...
| `skills/story-setup/references/templates/story/setting/*.md` | `{书名}/story/setting/*.md` | user state | create only if absent | never overwrite existing setting files |
...
- 仅当已识别为长篇书目且 `{书名}/story/` 已存在时，执行以下操作：
  1. 确保 `{书名}/story/setting/` 目录存在
  2. 依次读取 `skills/story-setup/references/templates/story/setting/*.md` 中的 8 个骨架模板
  3. 对每个模板文件，检查 `{书名}/story/setting/{同名文件}.md` 是否已存在
     - 不存在 → 将模板写入目标路径（替换占位符）
     - 已存在 → **不覆盖**，保留用户已有内容
  4. 如已存在 `story/setting/` 但只有部分文件，补充缺失的文件
- 短篇项目或无需长篇设定的题材，跳过此步骤
