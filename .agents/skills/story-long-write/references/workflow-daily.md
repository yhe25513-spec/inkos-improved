...
**⚠️ 项目可能使用不同的目录结构，开始写作前必须检测。**

1. 检测项目的实际目录结构：
   - 检查 `chapters/` 目录是否存在 → 项目使用 **webnovel-writer 格式**（`chapters/000X_章名.md`，跟踪文件在 `story/`）
   - 检查 `正文/` 目录是否存在 → 项目使用 **story-long-write 格式**（`正文/第00X章_章名.md`，跟踪文件在 `追踪/`）
   - 如果项目根下不存在上述目录，检查 `books/*/` 子目录中用 `ls books/*/chapters/ 2>/dev/null` 和 `ls books/*/正文/ 2>/dev/null` 确认
2. 记录检测结果，后续所有文件操作必须使用实际检测到的路径格式：
   ...
   ```
3. **确定下一章编号 N**（按优先级）：
   - 如果存在 `chapters/index.json`，读取最后一项的 `number` 字段，N = number + 1
   - 如果存在 `追踪/上下文.md`，读取"最后完成章节"字段
   - 否则，扫描 `chapters/` 或 `正文/` 目录中编号最大的章节 +1
...
> **路径说明**：下列路径根据 Step 0 检测结果适应。webnovel-writer 项目（有 `chapters/` 和 `story/`）使用 `story/current_focus.md`、`story/pending_hooks.md`、`story/runtime/` 等实际路径；story-long-write 项目（有 `正文/` 和 `追踪/`）使用 `追踪/` 路径。

| 序号 | 文件（优先路径 → 回退路径） | 用途 | 如果不存在 |
|------|------|------|-----------|
| 1 | `story/current_focus.md` → `追踪/上下文.md` | 上次写作进度摘要 | 从 `story/current_state.md` 或 `追踪/伏笔.md` + `追踪/时间线.md` 重建 |
| 2 | `story/pending_hooks.md` → `追踪/伏笔.md` | 待回收伏笔清单 | 跳过 |
| 3 | `story/current_state.md` → `追踪/时间线.md` | 当前事件时序 | 跳过（写作时从正文推断） |
| 4 | `story/runtime/chapter-{NNNN}.plan.md` → `大纲/细纲_第{N}章.md` | 本章写作计划 | **必须先补建**，不允许跳过 |

**按需加载角色文件**：从细纲中提取本章涉及的角色名，按需加载角色设定文件（优先 `story/roles/{类型}/{角色名}.md`，其次 `设定/角色/{角色名}.md`）。如果细纲中未列出角色，跳过。
...
> **确定下一章编号 N**（已移至 Step 0 检测阶段，此处为兜底回退）：扫描章节目录（`chapters/` 或 `正文/`）中编号最大的章节 +1。优先从 `chapters/index.json` 或 `追踪/上下文.md` 的"最后完成章节"字段读取。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.2 状态筛选**：每章开始前必须确认以下来源已经在本轮 workflow 中读取或刚更新：本章细纲、上一章正文（或上一章刚写入的正文）。必须确认跟踪文件（优先 `story/current_focus.md`、`story/pending_hooks.md`、`story/current_state.md`；回退 `追踪/上下文.md`、`追踪/伏笔.md`、`追踪/时间线.md`）；涉及角色时，还必须确认 `story/roles/{类型}/{角色名}.md` 或 `追踪/角色状态.md` 或 `设定/角色/{角色名}.md` 的来源。"已加载"只指本轮 workflow 内实际读取/更新过的文件，不得用未标明来源的聊天记忆替代。角色最新状态优先从 `story/current_state.md` 或 `追踪/角色状态.md` 筛选（如不存在则从角色设定推断），待回收/推进伏笔从 `story/pending_hooks.md` 或 `追踪/伏笔.md` 筛选；细纲不存在时仍按下方补建流程处理，不允许直接写正文
   - **2.3 对标模块/节奏/文风召回**：
   ...
   - **2.5 计划预览确认**：将 2.4 的意图确认汇总为一段简纲（核心事件+情绪目标+钩子+字数），展示给用户：
     ```
     第 N 章写作计划：
     - 核心事件：{一句话}
     - 情绪目标：{目标情绪}
     ...
     - 字数目标：{目标字数}
     ```
     询问用户是否确认。用户确认后进入写作；用户提出修改意见时，按意见调整意图后再写。
     如果用户已明确表示不需要逐章确认，或批量续写模式下用户已确认过整体方向，可跳过此步骤。
   ...
   - 每章写完后**立即更新**以下文件（按项目结构检测结果选择实际路径）：
     - 伏笔文件（优先 `story/pending_hooks.md`，回退 `追踪/伏笔.md`）（新增/回收伏笔）
     - 状态/时间线文件（优先 `story/current_state.md`，回退 `追踪/时间线.md`）（记录事件时序）
     - 角色状态文件（优先 `story/current_state.md` + 对应角色文件，回退 `追踪/角色状态.md`）（如本章引起角色状态变化——身份、能力、关系、公众形象——则更新对应角色条目并追加变更记录）
     - 进度文件（优先 `story/current_focus.md`，回退 `追踪/上下文.md`）（只更新进度元信息：当前位置+已写字数+本次变更，不写详细角色状态/伏笔内容）
     - **索引文件**：如果项目使用 `chapters/index.json`，每章完成后更新对应章节条目（`status`、`wordCount`、`updatedAt`）
   - **质检提示**（可选）：本章写作完成。如需一致性检查，运行 `/story-review lean`。批量写作模式跳过此步骤，全部写完后再统一审查。
   - **Pipeline 同步**：本章若通过 skill 系统直接写入文件（而非 sub_agent writer），写完后建议运行 `sub_agent(agent="auditor")` 触发审查管道，确保章节被纳入 index.json 和快照体系。
...
7. **确定性收尾**：对本批所有新写正文文件先运行 `node .agents/skills/story-long-write/scripts/normalize-punctuation.js $CHAPTER_FILE`（写模式，默认 `--quote-mode keep`），再运行 `node .agents/skills/story-long-write/scripts/check-ai-patterns.js --check $CHAPTER_FILE`（`$CHAPTER_FILE` 为实际写入的章节文件路径，如 `chapters/0002_龟兔与废铁.md` 或 `正文/第002章_龟兔与废铁.md`）。前者确定性去掉正文里的 `……`、破折号 `——`/`—`、双连字符 `--` 和独立行 `---`，避免长篇写到上百章后横线或长省略累积；后者只报告先否定再肯定的高危 AI 句式，命中时回正文改掉并复扫到 0。脚本不再保留对话打断破折号；数字区间会改写为中文连接词，盐言「」不受影响。**由谁运行**：narrative-writer agent 不运行这些脚本；主会话在 agent 写完返回后、针对实际落盘的正文文件路径运行（文件名是写完才确定的）。
